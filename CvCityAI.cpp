...
void CvCityAI::AI_chooseProduction()
{
	...
	//UVC: Why check for iPercentOfDomination to check the health? And why 90%?
	{
...
void CvCityAI::AI_stealPlots()
{
	...
	//UVC: I don't understand what this function is meant to do
	//UVC: It is one of the first functions called in AIdoTurn

    for (iI = 0; iI < NUM_CITY_PLOTS; iI++)
    {
        pLoopPlot = plotCity(getX_INLINE(),getY_INLINE(),iI);
		//UVC: plotCity is not defined in this code, I don't know where it is
        
        if (pLoopPlot != NULL)
        {
            if (iImportance > 0)
			//UVC: I have no idea why this if condition is outside of the loop
            {
            ...
            //UVC: Isn't this a very stupid way to check if the previous if happened?    
